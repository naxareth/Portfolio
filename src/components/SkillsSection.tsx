  ...
  return (
    <section id="skills" className="py-24 px-6 border-y border-border bg-surface/50 backdrop-blur-sm">
      <div className="max-w-6xl mx-auto">
        <h2 className="text-4xl font-serif mb-16 drop-shadow-md">What I Do</h2>
        ...
        <div className="flex flex-col">
          {skills.map((skill, index) => (
            ...
            >
              <div className="text-sm text-muted font-mono drop-shadow-sm">{skill.num}</div>
              <h3 className="text-2xl font-serif drop-shadow-sm">{skill.title}</h3>
              <p className="text-muted text-sm leading-relaxed drop-shadow-sm">{skill.desc}</p>
              <div className="hidden md:flex items-center justify-center w-10 h-10 rounded-full border border-border opacity-0 -translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 group-hover:-translate-y-1 transition-all duration-300 drop-shadow-sm">
                ↗
              </div>
            </motion.div>
          ))}
